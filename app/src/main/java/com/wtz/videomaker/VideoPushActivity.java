...
import com.wtz.libvideomaker.push.WeVideoPusher;
...
public class VideoPushActivity extends AppCompatActivity implements PermissionHandler.PermissionHandleListener,
    ...
    private WeVideoPusher mWeVideoPusher;
    ...
    static class WeakHandler extends Handler {
        private final WeakReference<VideoPushActivity> weakReference;

        // 为了避免非静态的内部类和匿名内部类隐式持有外部类的引用，改用静态类
        // 又因为内部类是静态类，所以不能直接操作宿主类中的方法了，
        // 于是需要传入宿主类实例的弱引用来操作宿主类中的方法
        public WeakHandler(VideoPushActivity host) {
        ...
        public void handleMessage(Message msg) {
            VideoPushActivity host = weakReference.get();
            if (host == null) {
                return;
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ScreenUtils.hideNavigationBar(VideoPushActivity.this); // 隐藏导航栏
        needHideNav = false;

        setContentView(R.layout.activity_video_push);
        initViews();

        mPushUrl = "rtmp://192.168.0.101/myapp/mystream";
        mWeVideoPusher = new WeVideoPusher(this);
        mWeVideoPusher.setPushUrl(mPushUrl);
        mWeVideoPusher.setConnectTimeout(7);
    ...
    private void initViews() {
        ...
        mPushButton = findViewById(R.id.btn_record);
    ...
    private WeNAudioRecorder.OnAudioRecordDataListener mNAudioListener = new WeNAudioRecorder.OnAudioRecordDataListener() {
        ...
        public void onAudioRecordData(byte[] data, int size) {
            ...
            if (mAudioSrcType == AUDIO_SRC_TYPE_MIC_MUSIC) {
                synchronized (VideoPushActivity.this) {
                    isMixing = true;
                }
                ...
                putAudioToVideoPusher(mixOut, size);

                synchronized (VideoPushActivity.this) {
                    isMixing = false;
                }
            } else {
                putAudioToVideoPusher(data, size);
            }
        }
    };

    private void putAudioToVideoPusher(byte[] data, int size) {
        if (mWeVideoPusher != null) {
            mWeVideoPusher.enqueueAudioData(data, size);
        }
    }
    ...
    private void startRecord() {
        ...
        mPushButton.setText(R.string.stop_live);
    ...
    private void startVideoEncode(boolean hasAudio) {
        if (hasAudio) {
            mWeVideoPusher.setAudioParams(mSampleRate, mChannelNums, mBitsPerSample, mPcmMaxBytesPerCallback);
        }
        mWeVideoPusher.setExternalTextureId(mWeCameraView.getScreenTextureId());
        mWeVideoPusher.startPush(mWeCameraView.getSharedEGLContext(),
    ...
    public void onPCMDataCall(byte[] bytes, int size) {
        if (mAudioSrcType == AUDIO_SRC_TYPE_MIC_MUSIC) {
            ...
            putAudioToVideoPusher(bytes, size);
        }
    }
    ...
    private void stopRecord() {
        ...
        mWeVideoPusher.stopPush();

        stopUpdateRecordInfo();
        mPushButton.setText(R.string.start_live);
    ...
    private void updateRecordInfo() {
        if (mWeVideoPusher != null) {
            String time = DateTimeUtil.changeRemainTimeToHms(mWeVideoPusher.getEncodeTimeMills());
    ...
    public void onCameraSizeChanged(int surfaceWidth, int surfaceHeight, int previewWidth, int previewHeight) {
        if (mWeVideoPusher != null) {
            mWeVideoPusher.onVideoSizeChanged(surfaceWidth, surfaceHeight);
        }
    }
    ...
    public void onScreenTextureChanged(int textureId) {
        ...
        if (mWeVideoPusher != null) {
            mWeVideoPusher.setExternalTextureId(textureId);
        }
    }
